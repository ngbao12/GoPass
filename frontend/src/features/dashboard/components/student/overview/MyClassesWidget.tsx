...
const JoinClassModal: React.FC<JoinClassModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl w-full max-w-md shadow-xl transform transition-all scale-100"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 pt-2">
          ...
          <button
            ...
            className="w-full py-3 px-4 rounded-lg font-bold text-white bg-teal-500 hover:bg-teal-600 shadow-lg disabled:opacity-50 transition-all"
          >
  ...
  const handleJoinSubmit = async (code: string) => {
    ...
    if (result.success) {
      ...
      if (result.error === "NOT_FOUND") {
        setNotification({
          isOpen: true,
          message: `Class code "${code}" does not exist.`,
          type: "error",
        });
      } else if (result.error === "EXISTED") {
        setNotification({
          isOpen: true,
          message: "You have already joined or sent a request to this class.",
          type: "warning",
        });
      } else {
        setNotification({
      ...
      onConfirm: async () => {
        ...
        if (success) {
          ...
          setNotification({
            isOpen: true,
            message:
              "Không thể hủy yêu cầu. Có thể giáo viên đã phê duyệt yêu cầu này rồi.",
            type: "error",
          });
        }
      },
      ...
      <div className="bg-white p-6 rounded-xl border border-teal-50 shadow-sm h-full flex flex-col max-h-[700px]">
        ...
        <div className="mb-4 relative shrink-0">
          <input
            type="text"
            placeholder={
              isViewPending ? "Tìm kiếm lớp đang chờ..." : "Tìm kiếm lớp học..."
            }
            value={searchQuery}
        ...
        <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
          {isLoadingData ? (
            ...
            [1, 2, 3].map((i) => (
              <div
                key={i}
                className="animate-pulse flex items-center p-3 border border-gray-100 rounded-lg"
              >
            ...
            filteredList.map((cls) =>
              isViewPending ? (
                ...
                <div
                  key={cls.id}
                  className="bg-orange-50 border border-orange-100 rounded-lg p-3 flex justify-between items-center animate-in fade-in slide-in-from-bottom-1 duration-300"
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-500 flex items-center gap-1">
                        <span>{cls.teacher || "Giáo viên"}</span> •{" "}
                        <span>{cls.requestDate}</span>
                      </p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    ...
                    <button
                      onClick={() =>
                        cls.requestId && handleCancelRequest(cls.requestId)
                      }
