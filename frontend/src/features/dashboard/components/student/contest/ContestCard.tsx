...
const ContestCard: React.FC<ContestCardProps> = ({
  ...
  return (
    <div
      className={`bg-white p-5 rounded-xl border ${borderColor} ${cardOpacity} transition-all`}
    >
      <div className="flex flex-col md:flex-row justify-between gap-4 items-center">
        ...
        <div className="shrink-0 w-full md:w-auto">
          {isOngoing && (
            <>
              {contest.progress &&
              contest.progress.completed === contest.progress.total &&
              contest.progress.total > 0 ? (
                <button
                  className="w-full md:w-auto px-6 py-2 border border-green-200 text-green-700 font-bold text-sm rounded-lg bg-green-50 cursor-not-allowed flex items-center justify-center gap-2"
                  disabled
                >
                  <svg
                    className="w-4 h-4"
