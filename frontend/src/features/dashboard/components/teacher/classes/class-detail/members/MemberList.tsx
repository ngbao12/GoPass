      ...
      {members.map((member) => (
        <div key={member._id} className="p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                // Ensure we convert to string if it's an ObjectId
                const memberId = String(
                  member.studentUserId || member.student?._id || ""
                );
                console.log(
                  "Remove button clicked! memberId:",
                  memberId,
                ...
                if (memberId) {
                  ...
                  console.error(
                    "Cannot remove member: missing studentUserId",
                    member
                  );
                }
              }}
              disabled={
                actionLoading ===
                String(member.studentUserId || member.student?._id || "")
              }
              className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50 relative z-10 cursor-pointer"
              type="button"
            >
              <svg
                className="w-4 h-4"
