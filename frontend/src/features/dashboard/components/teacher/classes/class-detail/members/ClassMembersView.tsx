...
const ClassMembersView: React.FC<ClassMembersViewProps> = ({ classDetail, onUpdate }) => {
  ...
  const handleViewStats = async (member: any) => {
    try {
      ...
      const response = await classApi.getStudentStats(classDetail.id, studentId);
      if (response?.success) {
      ...
      <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-xl">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
              <h4 className="text-lg font-semibold text-gray-900">Thống kê học sinh</h4>
              <p className="text-sm text-gray-600">{selectedMember.student?.name || 'Học sinh'}</p>
            </div>
            <button onClick={closeStatsModal} className="p-2 text-gray-500 hover:text-gray-700">
              ...
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-teal-50 rounded-lg p-3">
                    ...
                    <p className="text-xl font-semibold text-teal-800">{memberStats.completedAssignments}/{memberStats.totalAssignments}</p>
                  </div>
                  <div className="bg-amber-50 rounded-lg p-3">
                    <p className="text-sm text-amber-700">Chưa làm</p>
                    <p className="text-xl font-semibold text-amber-800">{Math.max((memberStats.totalAssignments || 0) - (memberStats.completedAssignments || 0), 0)}/{memberStats.totalAssignments}</p>
                  </div>
                  <div className="bg-purple-50 rounded-lg p-3">
                    <p className="text-sm text-purple-700">Điểm trung bình</p>
                    <p className="text-xl font-semibold text-purple-800">{memberStats.averageScore ?? 0}</p>
                  </div>
                  <div className="bg-blue-50 rounded-lg p-3">
                    <p className="text-sm text-blue-700">Tổng bài</p>
                    <p className="text-xl font-semibold text-blue-800">{memberStats.totalAssignments || 0}</p>
                  </div>
                </div>
                ...
                {Array.isArray(memberStats.recentResults) && memberStats.recentResults.length > 0 ? (
                  ...
                  <div className="text-sm text-gray-500">Chưa có dữ liệu bài làm gần đây</div>
                )}
              </>
            ) : (
              <div className="text-sm text-gray-500">Không thể tải thống kê</div>
            )}
          </div>
        </div>
      </div>
