...
class SubmissionService {
  ...
  async startExam(assignmentId, studentUserId) {
    ...
    const existingSubmission =
      await ExamSubmissionRepository.findInProgressSubmission(
        assignmentId,
        studentUserId
      );
    ...
    // Check attempts
    const attempts = await ExamSubmissionRepository.getStudentAttempts(
      assignmentId,
      studentUserId
    );
    ...
    // Calculate max score
    const maxScore = await ExamQuestionRepository.calculateTotalScore(
      assignment.examId
    );
  ...
  async saveAnswer(submissionId, dto) {
    ...
    const answer = await ExamAnswerRepository.upsertAnswer(
      submissionId,
      questionId,
      {
        answerText,
  ...
  async submitExam(
    ...
    console.log("🔍 SubmissionService.submitExam called:", {
      submissionId,
      studentUserId,
    ...
    const savedAnswers = await ExamAnswerRepository.findBySubmission(
      submissionId
    );

    // Get all questions with correct answers
    const examQuestions = await ExamQuestionRepository.findByExam(
      submission.examId,
      {
        populate: "questionId",
      }
    ...
    const updatedSubmission = await ExamSubmissionRepository.findById(
      submissionId
    );

    // Trigger auto-grading for Ngữ Văn exams
    try {
      const exam = await ExamRepository.findById(submission.examId);
      if (exam && exam.subject === "Ngữ Văn") {
        console.log("📝 Triggering auto-grading for Ngữ Văn exam...");
        // Run auto-grading asynchronously without blocking response
        setImmediate(async () => {
          try {
            const gradingResult = await GradingService.autoGradeNguVan(
              submissionId
            );
            console.log("✅ Auto-grading completed:", {
              submissionId,
              gradedCount: gradingResult.gradedCount,
      ...
      console.error("❌ Error checking exam subject for auto-grading:", error);
    }
